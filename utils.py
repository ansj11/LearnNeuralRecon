...
class SaveScene(object):
    ...
    def reset(self):
        ...
        # self.coords = coordinates(np.array([416, 416, 128])).float() # 体大小, 三维网格坐标3x22151168（416x416x128）

        # for scale in range(self.cfg.MODEL.N_LAYER): # 3 三个尺度
        #     s = 2 ** (self.cfg.MODEL.N_LAYER - scale - 1) # 4, 2 1
        #     dim = tuple(np.array([416, 416, 128]) // s) # 维度
        #     self.tsdf_volume.append(torch.ones(dim).cuda()) # tsdf体初始化为1
        #     self.weight_volume.append(torch.zeros(dim).cuda()) # 融合权重初始化为0
    ...
    def tsdf2mesh(voxel_size, origin, tsdf_vol):
        verts, faces, norms, vals = measure.marching_cubes_lewiner(tsdf_vol, level=0) # verts faces norms: nx3, vals: nx1 n=69402, faces:138832x3
        verts = verts * voxel_size + origin  # voxel grid coordinates to world coordinates 一个点的size=0.04m，origin偏移量
    ...
    def save_scene_eval(self, epoch, outputs):
        ...
        origin = outputs['origin'][0].data.cpu().numpy() # -0.2，-0.28，-0.08

        if (tsdf_volume == 1).all():
            logger.warning('No valid data for scene {}'.format(self.scene_name)) # 全是1则不对
        else:
            # Marching cubes
            mesh = self.tsdf2mesh(self.cfg.MODEL.VOXEL_SIZE, origin, tsdf_volume) # 转成mesh:verts,faces,norms
            # save tsdf volume for atlas evaluation
            data = {'origin': origin, # 偏置
                    'voxel_size': self.cfg.MODEL.VOXEL_SIZE, # 0.04m
                    'tsdf': tsdf_volume}    # 体
            ...
            np.savez_compressed(
                ...
                **data) # save data
            mesh.export(os.path.join(save_path, '{}.ply'.format(self.scene_name))) # save mesh
...
=======
            scene = scene.replace('/', '-')
            # self.scene_name = None: [True, False, True]
            if scene != self.scene_name and self.scene_name is not None and self.cfg.SAVE_SCENE_MESH:
                print('新场景%s != %s，加入scene_tsdf，并重建mesh'%(scene, self.scene_name))
                self.tsdf_volume.append(outputs['scene_tsdf']) # 如果是新场景，则加入tsdf
                self.save_scene_eval(epoch_idx, outputs)

            if self.scene_name is None or scene != self.scene_name: # [True, True]
                print('初始化tsdf为1，权重为0')
                self.scene_name = scene # 第二帧不再进入此处
                self.reset() # 初始化tsdf为1，融合权重初始化为0
            else:
                self.keyframe_id += 1
